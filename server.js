...
async function fetchNbaStandings() {
  ...
  const standingsStats = rows.map((row, i) => {
    const teamId = String(row[idx('TeamID')] || '');
    const abbr = String(row[idx('TeamAbbreviation')] || NBA_TEAM_ID_TO_TRICODE[teamId] || '').toUpperCase();
...
async function buildNbaLiveData() {
  ...
  const hasValidStandings = standingsData.standingsStats.length > 0 && standingsData.standingsStats.some(team => team.code);
  let finalStandings = standingsData.standingsStats;
  let finalTable = standingsData.teamStandingsTable;

  if (!hasValidStandings) {
    const nbaTeamSet = new Set(Object.values(NBA_TEAM_ID_TO_TRICODE));
    const teamRecs = {};
    for (const game of allGames) {
      if (!game.played) continue;
      const homeCode = game.home?.code;
      const awayCode = game.away?.code;
      if (!homeCode || !awayCode || !nbaTeamSet.has(homeCode) || !nbaTeamSet.has(awayCode)) continue;
      if (!teamRecs[homeCode]) teamRecs[homeCode] = { code: homeCode, w: 0, l: 0, homeW: 0, homeL: 0, awayW: 0, awayL: 0, last10: [], pts: 0, ptsA: 0, conference: '', rank: 99, confRank: 99 };
      if (!teamRecs[awayCode]) teamRecs[awayCode] = { code: awayCode, w: 0, l: 0, homeW: 0, homeL: 0, awayW: 0, awayL: 0, last10: [], pts: 0, ptsA: 0, conference: '', rank: 99, confRank: 99 };

      if (game.home.score > game.away.score) {
        teamRecs[homeCode].w++;
        teamRecs[homeCode].homeW++;
        teamRecs[homeCode].last10.push('W');
        teamRecs[awayCode].l++;
        teamRecs[awayCode].awayL++;
        teamRecs[awayCode].last10.push('L');
      } else {
        teamRecs[awayCode].w++;
        teamRecs[awayCode].awayW++;
        teamRecs[awayCode].last10.push('W');
        teamRecs[homeCode].l++;
        teamRecs[homeCode].homeL++;
        teamRecs[homeCode].last10.push('L');
      }
    }

    const eastTeams = new Set(['BOS', 'NYK', 'CLE', 'MIL', 'IND', 'MIA', 'PHI', 'CHI', 'ATL', 'BKN', 'TOR', 'CHA', 'DET', 'ORL', 'WAS']);
    const eastStandings = Object.values(teamRecs).filter(team => eastTeams.has(team.code)).sort((a, b) => b.w - a.w);
    const westStandings = Object.values(teamRecs).filter(team => !eastTeams.has(team.code)).sort((a, b) => b.w - a.w);
    ...
    finalStandings = [...eastStandings, ...westStandings].sort((a, b) => b.w - a.w);
    finalStandings.forEach((team, index) => {
      team.rank = index + 1;
      team.last10 = team.last10.slice(-10);
    });
    finalTable = Object.fromEntries(finalStandings.map(team => [team.code, team.rank]));
  }
  ...
  return {
    ...
    teamStandingsTable: finalTable,
    standingsStats: finalStandings,
